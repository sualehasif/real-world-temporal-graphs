...
void node_statistics(const 
    std::vector<std::tuple<node_t, node_t, timestamp_t>> &edges,
    uint64_t print_freq, std::string &output_filename, const std::vector<node_t> nodes_to_track, bool use_real_time) {
...
template <class node_t = uint32_t, class timestamp_t = uint32_t>
std::vector<node_t> get_nodes_to_track(const 
    std::vector<std::tuple<node_t, node_t, timestamp_t>> &edges, const std::string command, node_t graph_nodes, size_t num_track) {
      if (command == "random") {
        ...
        std::partial_sort(counts.begin(), counts.begin() + num_track, counts.end(), std::greater{});
...
int main(int32_t argc, char *argv[]) {
  ...
  if (!absl::GetFlag(FLAGS_remove)) {
    auto [edges, graph_nodes] = AppendOnlyGraph<uint32_t>::get_graph_edges(absl::GetFlag(FLAGS_input));
    std::cout << "done loading the graph" << std::endl;
    std::cout << "have " << edges.size() << " edges and " << graph_nodes << " nodes"
              << std::endl;
    std::cout << " printing every " << print_freq << std::endl;
    if (graph_nodes > 0) {
      auto nodes_to_track = get_nodes_to_track(edges, absl::GetFlag(FLAGS_command), graph_nodes, absl::GetFlag(FLAGS_num_nodes));
      std::cout << "output filename is " << output_filename << std::endl;
      if (nodes_to_track.size() > 0) {
        node_statistics(edges, print_freq, output_filename, nodes_to_track, absl::GetFlag(FLAGS_use_real_time));
      }
    }
  } else {
